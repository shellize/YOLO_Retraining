...
GPU_ID="${GPU_ID:-0}"
STUDY_ROOT="${BALANCE_LEARNING_CURVE_ROOT:-$PROJECT_ROOT/runs/studies/balance_learning_curve_2batch}"
SEQUENCE_ROOT="$STUDY_ROOT/sequences"
REPORT_ROOT="$PROJECT_ROOT/charts/balance_learning_curve_2batch"
LOG_ROOT="$STUDY_ROOT/logs"
CONFIG_ROOT="$PROJECT_ROOT/configs/sequence/learning_curve"
...
conda run --no-capture-output -n "$CONDA_ENV" \
  python "$REPORT_ROOT/generate_report.py" \
