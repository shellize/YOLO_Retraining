...
def write_id_file(sample_ids: Sequence[str], path: Path) -> Path:
...
def write_backend_data_yaml(train_manifest: Path, val_manifest: Path, names: Sequence[str], path: Path) -> Path:
    payload = {"train": str(train_manifest.resolve()), "val": str(val_manifest.resolve()), "names": {index: name for index, name in enumerate(names)}}
    path.parent.mkdir(parents=True, exist_ok=True)
