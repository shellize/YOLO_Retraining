...
Task referenced under `tasks/full_raw__existing_result`.
...
- `build_manifests.py`: generates the four tau=0.90 variants and links the
  existing baseline.
- `variants/<variant>/layout.yaml`: complete train/validation/test layout.
- `variants/<variant>/manifests/train.txt`: merged training manifest.
- `variants/<variant>/manifests/stage0.txt` through `stage3.txt`: logical stage
  partitions used by the Task config.
- `variants/<variant>/manifests/val.txt` and `test.txt`: fixed balance holdouts.
- `clusters_tau_0p900.csv`: cluster membership and representative audit.
- `manifest_summary.json`: generated image/cluster counts and baseline reference.
- `run_experiments.sh`: serial launcher for the four unfinished Tasks.
- `run_when_gpu0_idle.sh`: polls GPU0 every three minutes and starts training
...
tmux new-session -d -s balance_tau090_label_contrast \
  "bash runs/studies/balance_tau090_label_contrast/run_when_gpu0_idle.sh"
```
