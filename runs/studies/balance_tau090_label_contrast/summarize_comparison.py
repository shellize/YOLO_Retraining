...
def load_experiment(spec: dict[str, str | Path]) -> dict[str, object]:
    result_dir = Path(spec.get("result_dir", STUDY_ROOT / "tasks" / str(spec["name"]))).resolve()
...
def write_outputs(rows: list[dict[str, object]]) -> None:
    csv_path = STUDY_ROOT / "comparison.csv"
    fields = [
        "name",
        "label",
        "protocol",
    ...
    (STUDY_ROOT / "comparison.md").write_text("\n".join(markdown) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    for spec in EXPERIMENTS:
        if "result_dir" not in spec:
            spec = {
                **spec,
                "result_dir": STUDY_ROOT / "tasks" / f"{spec['name']}{TASK_SUFFIX}",
                "log": STUDY_ROOT / "logs" / f"{spec['name']}.log",
            }
        rows.append(load_experiment(spec))
    write_outputs(rows)
    print((STUDY_ROOT / "comparison.md").resolve())
    print((STUDY_ROOT / "comparison.csv").resolve())
    return 0
