...
a different train/validation/test layout. `scripts/build_manifests.py` remaps cached
...
Each generated `experiment/variants/<name>/layout.yaml` contains `stage0` through `stage7`
...
`experiment/variants/<name>/clusters.csv`; aggregate counts are in
`experiment/variants/manifest_summary.json`.
...
tmux new-session -d -s balance_lc2b_dedup \
  "bash runs/studies/0902_dedup_balance_learning_curve_2batch/scripts/run_when_gpu0_idle.sh"
```
