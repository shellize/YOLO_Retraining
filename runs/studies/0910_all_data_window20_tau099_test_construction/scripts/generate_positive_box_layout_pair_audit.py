...
</style></head><body><main><h1>正样本相邻帧 · 框布局双图审阅</h1><div class="muted">候选条件：全局帧号相差 1、两张图都有有效标注、总框数相同。框布局距离 = 最优一一匹配后的平均〔中心距离 + 0.5 × 宽高距离〕，坐标均已归一化；越小只表示框布局越接近，不等同于必然重复。类别不参与匹配，以免漏掉标注类别不一致的近重复帧。</div>
...
const DATA={data},ROOT={root},COLORS={colors},KEY='0910_positive_box_layout_pair_audit_v1';let review=JSON.parse(localStorage.getItem(KEY)||'{{}}'),index=0;const $=id=>document.getElementById(id),esc=x=>String(x).replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c])),save=()=>localStorage.setItem(KEY,JSON.stringify(review));
...
$('export').onclick=()=>{{const q=v=>'"'+String(v??'').replaceAll('"','""')+'"',out=[['rank','left','right','left_frame','right_frame','batch','box_count','left_signature','right_signature','layout_distance','mean_matched_iou','class_mismatches','cosine_similarity','left_split','right_split','cross_split','status','note']];for(const row of DATA.pairs){{const r=review[row.rank]||{{}};out.push([row.rank,row.left.image,row.right.image,row.left_frame,row.right_frame,row.batch,row.box_count,row.left_signature,row.right_signature,row.layout_distance,row.mean_matched_iou,row.class_mismatches,row.cosine_similarity,row.left.split,row.right.split,row.cross_split?1:0,r.status||'',r.note||''])}}const blob=new Blob(['\ufeff'+out.map(row=>row.map(q).join(',')).join('\\n')],{{type:'text/csv;charset=utf-8'}}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='positive_box_layout_pair_manual_review.csv';a.click();URL.revokeObjectURL(a.href)}};$('clear').onclick=()=>{{if(confirm('清空当前浏览器保存的全部审阅？')){{review={{}};save();render()}}}};document.addEventListener('keydown',e=>{{if(e.target.matches('input,textarea,select'))return;if(e.key==='ArrowLeft')$('prev').click();if(e.key==='ArrowRight')$('next').click()}});render();
</script></body></html>'''
...
def main() -> int:
    ...
    for left_path, right_path in zip(images, images[1:]):
        if frame(right_path) - frame(left_path) != 1:
            continue
    ...
    with (output_dir / "candidate_pairs.csv").open("w", encoding="utf-8-sig", newline="") as handle:
        fieldnames = [
            "rank", "left", "right", "left_frame", "right_frame", "batch", "box_count",
    ...
    summary = {
        "status": "completed",
        "candidate_rule": "global frame gap == 1; both positive; equal total box counts",
        ...
        "browser_export": "positive_box_layout_pair_manual_review.csv",
    }
