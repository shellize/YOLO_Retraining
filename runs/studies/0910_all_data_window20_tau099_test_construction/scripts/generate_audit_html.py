...
def load_clusters(path: Path) -> list[dict[str, object]]:
                ...
                "members": [
                    {
                        ...
                        "similarity": float(row["score_to_ordinary_representative"]),
                    }
                    for row in rows
                ],
            }
    ...
    clusters.sort(
        key=lambda cluster: (
            -int(cluster["size"]),
            -max(float(member["similarity"]) for member in cluster["members"] if member["image"] != cluster["ordinary_representative"]),
            str(cluster["id"]),
        )
    )
    return clusters
...
<title>全量数据 window=20 时序去重簇审计</title><style>
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with (post_dir / "top_similar_pairs.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["rank", "left", "right", "similarity", "same_batch"],
        )
        writer.writeheader()
        for pair in pairs:
    ...
    post_summary = {
        ...
        "manual_review_export": "post_dedup_manual_review.csv (downloaded by the browser)",
    }
