...
:root{{--bg:#f4f7fb;--surface:#fff;--ink:#172033;--muted:#647087;--line:#d9e0eb;--blue:#2563eb;--green:#15803d;--red:#b91c1c;--orange:#c2410c;--shadow:0 10px 28px rgba(32,50,86,.08)}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:14px/1.45 "Segoe UI","Microsoft YaHei",sans-serif}}main{{max-width:1560px;margin:auto;padding:24px}}h1{{margin:0 0 6px;font-size:26px}}.muted{{color:var(--muted)}}.summary,.toolbar,.review,.pages{{display:flex;flex-wrap:wrap;gap:9px;align-items:center}}.summary{{margin:18px 0}}.metric,.pair{{background:var(--surface);border:1px solid var(--line);border-radius:12px;box-shadow:var(--shadow)}}.metric{{min-width:155px;padding:10px 13px}}.metric small{{display:block;color:var(--muted)}}.metric strong{{font-size:21px}}button,select,input{{font:inherit;border:1px solid var(--line);background:#fff;border-radius:8px;padding:7px 10px}}button{{cursor:pointer}}.toolbar{{position:sticky;top:0;z-index:8;background:rgba(244,247,251,.95);backdrop-filter:blur(8px);padding:12px 0}}#pairs{{display:grid;gap:12px}}.pair{{padding:12px}}.pair.exact{{border:2px solid var(--orange)}}.head{{display:flex;justify-content:space-between;gap:12px;margin-bottom:9px}}.score{{font-weight:700;color:var(--blue)}}.images{{display:grid;grid-template-columns:1fr 1fr;gap:12px}}.tile .figure{{position:relative;background:#e9eef5;cursor:zoom-in}}.tile img{{display:block;width:100%;height:auto}}.tile svg{{position:absolute;inset:0;width:100%;height:100%;pointer-events:none}}.box{{fill-opacity:.07;stroke-width:.006}}.caption{{font-size:12px;color:var(--muted);padding:5px 1px;overflow-wrap:anywhere}}.review{{margin-top:8px;padding-top:9px;border-top:1px solid var(--line)}}.choice.active[data-status=near_duplicate]{{background:var(--red);color:#fff}}.choice.active[data-status=same_event_distinct]{{background:var(--orange);color:#fff}}.choice.active[data-status=different]{{background:var(--green);color:#fff}}.choice.active[data-status=unsure]{{background:var(--blue);color:#fff}}textarea{{width:100%;min-height:45px;border:1px solid var(--line);border-radius:8px;padding:7px;font:inherit}}.tile.big{{grid-column:1/-1}}.tile.big .figure{{max-width:1200px}}.pages{{margin:16px 0}}.pages button[aria-current=page]{{background:var(--blue);color:#fff}}@media(max-width:760px){{main{{padding:14px}}.images{{grid-template-columns:1fr}}}}
...
<div class="toolbar"><select id="status"><option value="all">全部状态</option><option value="unreviewed">未审阅</option><option value="near_duplicate">连续帧 / 近重复</option><option value="same_event_distinct">同事件但应保留</option><option value="different">不同事件</option><option value="unsure">不确定</option></select><select id="scope"><option value="all">全部相邻对</option><option value="same_timestamp">仅同时间戳</option><option value="gap1">仅帧距 1</option><option value="cross_split">仅跨 split</option><option value="both_positive">仅双方正样本</option><option value="either_positive">至少一方正样本</option></select><select id="batch"><option value="all">全部 batch</option></select><select id="sort"><option value="timestamp_first">同时间戳优先，其次帧距</option><option value="gap">帧距升序</option><option value="similarity">相似度降序</option></select><input id="search" placeholder="搜索文件名"><button id="export">导出审阅 CSV</button><button id="clear">清空审阅</button><span class="muted" id="shown"></span></div><section id="pairs"></section><div class="pages" id="pages"></div>
</main><script>
const DATA={data},ROOT={root},COLORS={colors},KEY='0910_retained_temporal_pair_audit_v1',SIZE=20;let review=JSON.parse(localStorage.getItem(KEY)||'{{}}'),page=1;const $=id=>document.getElementById(id),esc=x=>String(x).replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c])),save=()=>localStorage.setItem(KEY,JSON.stringify(review));
...
function rows(){{let out=DATA.pairs.slice(),s=$('status').value,scope=$('scope').value,b=$('batch').value,q=$('search').value.trim().toLowerCase();if(s==='unreviewed')out=out.filter(r=>!review[r.rank]?.status);else if(s!=='all')out=out.filter(r=>review[r.rank]?.status===s);if(scope==='same_timestamp')out=out.filter(r=>r.same_timestamp);if(scope==='gap1')out=out.filter(r=>r.frame_gap===1);if(scope==='cross_split')out=out.filter(r=>r.cross_split);if(scope==='both_positive')out=out.filter(r=>r.both_positive);if(scope==='either_positive')out=out.filter(r=>r.either_positive);if(b!=='all')out=out.filter(r=>r.batch===b);if(q)out=out.filter(r=>r.left.toLowerCase().includes(q)||r.right.toLowerCase().includes(q));const mode=$('sort').value;if(mode==='timestamp_first')out.sort((a,b)=>Number(b.same_timestamp)-Number(a.same_timestamp)||a.frame_gap-b.frame_gap||b.similarity-a.similarity);if(mode==='gap')out.sort((a,b)=>a.frame_gap-b.frame_gap||b.similarity-a.similarity);if(mode==='similarity')out.sort((a,b)=>b.similarity-a.similarity||a.frame_gap-b.frame_gap);return out}}
...
function card(row){{const r=review[row.rank]||{{status:'',note:''}},chain=row.chain_id?` · 连续链 ${{row.chain_id}}（${{row.chain_size}} 张）`:'';return `<article class="pair ${{row.same_timestamp?'exact':''}}" data-rank="${{row.rank}}"><div class="head"><strong>#${{row.rank}} · 帧距 ${{row.frame_gap}} · ${{esc(row.batch)}} · ${{esc(row.left_split)}}/${{esc(row.right_split)}}${{row.same_timestamp?' · 同时间戳':''}}${{chain}}</strong><span class="score">similarity=${{row.similarity.toFixed(8)}}</span></div><div class="images">${{tile(row.left)}}${{tile(row.right)}}</div><div class="review"><button class="choice ${{r.status==='near_duplicate'?'active':''}}" data-status="near_duplicate">连续帧 / 近重复</button><button class="choice ${{r.status==='same_event_distinct'?'active':''}}" data-status="same_event_distinct">同事件但应保留</button><button class="choice ${{r.status==='different'?'active':''}}" data-status="different">不同事件</button><button class="choice ${{r.status==='unsure'?'active':''}}" data-status="unsure">不确定</button><textarea placeholder="备注">${{esc(r.note||'')}}</textarea></div></article>`}}
function render(){{const all=rows(),total=Math.max(1,Math.ceil(all.length/SIZE));page=Math.min(page,total);$('pairs').innerHTML=all.slice((page-1)*SIZE,page*SIZE).map(card).join('');$('shown').textContent=`显示 ${{all.length}} / ${{DATA.pairs.length}} 对 · 第 ${{page}}/${{total}} 页`;$('reviewed').textContent=Object.values(review).filter(x=>x.status).length;$('pages').innerHTML='';for(let i=1;i<=total;i++){{if(total>14&&i>2&&i<total-1&&Math.abs(i-page)>1)continue;const b=document.createElement('button');b.textContent=i;b.setAttribute('aria-current',i===page?'page':'false');b.onclick=()=>{{page=i;render();scrollTo({{top:0,behavior:'smooth'}})}};$('pages').appendChild(b)}}}}
for(const id of ['status','scope','batch','sort'])$(id).onchange=()=>{{page=1;render()}};$('search').oninput=()=>{{page=1;render()}};$('pairs').onclick=e=>{{const pair=e.target.closest('.pair');if(!pair)return;const choice=e.target.closest('.choice');if(choice){{review[pair.dataset.rank]=review[pair.dataset.rank]||{{status:'',note:''}};review[pair.dataset.rank].status=choice.dataset.status;save();render();return}}const tile=e.target.closest('.tile');if(tile)tile.classList.toggle('big')}};$('pairs').oninput=e=>{{if(e.target.tagName!=='TEXTAREA')return;const rank=e.target.closest('.pair').dataset.rank;review[rank]=review[rank]||{{status:'',note:''}};review[rank].note=e.target.value;save()}};
$('export').onclick=()=>{{const q=v=>'"'+String(v??'').replaceAll('"','""')+'"',out=[['rank','left','right','frame_gap','similarity','same_timestamp','chain_id','chain_size','left_split','right_split','status','note']];for(const row of DATA.pairs){{const r=review[row.rank]||{{}};out.push([row.rank,row.left,row.right,row.frame_gap,row.similarity,row.same_timestamp?1:0,row.chain_id,row.chain_size,row.left_split,row.right_split,r.status||'',r.note||''])}}const blob=new Blob(['\ufeff'+out.map(r=>r.map(q).join(',')).join('\\n')],{{type:'text/csv;charset=utf-8'}}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='retained_temporal_pair_manual_review.csv';a.click();URL.revokeObjectURL(a.href)}};$('clear').onclick=()=>{{if(confirm('清空当前浏览器保存的全部审阅？')){{review={{}};save();render()}}}};$('batch').innerHTML='<option value="all">全部 batch</option>'+[...new Set(DATA.pairs.map(r=>r.batch))].sort().map(b=>`<option>${{esc(b)}}</option>`).join('');render();
</script></body></html>'''
...
def main() -> int:
    ...
    summary = {
        ...
        "browser_export": "retained_temporal_pair_manual_review.csv",
    }
