...
正式划分冻结后，将 `stage0..stage7`、`val`、完整 `test`、`test_filtered` 和 `test_difficult` 的图片身份发布到公共 `configs/data/manifests/self_improving/`，并由公共 `configs/data/self_improving.yaml` 逐组引用。后续默认训练不再用目录扫描决定样本身份，所有组均以 TXT manifest 为唯一约束；这一步只发布既有身份，不再次划分或移动数据。
...
由于人工审计发现部分标注框边界不够准确，正式评测新增 AP@0.1、AP@0.2 和 AP@0.3，同时继续保留标准 AP@0.5 与 AP@0.5:0.95。默认主指标和验证集 `best.pt` 选择标准均改为 AP@0.3，Precision/Recall 及固定置信度诊断也以 IoU 0.3 判定命中。每个被评测的 test 图片保存标注框、预测框、置信度和各 IoU 阈值的匹配结果，便于不重新推理地检查完整 test 及其子集。
