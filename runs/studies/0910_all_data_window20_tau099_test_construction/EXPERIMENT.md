...
两个页面都引用仓库 `data/self_improving/` 中的原图。Codex 的隔离 HTML 预览以及部分浏览器的直接 `file://` 打开方式可能只显示页面结构而阻止跨目录本地图片；Windows 下应在仓库根目录运行以下入口，通过仅绑定 `127.0.0.1` 的临时 HTTP 服务查看：

```powershell
.\runs\studies\0910_all_data_window20_tau099_test_construction\scripts\open_audit.cmd
```

默认打开去重簇页面；传入 `--page pairs` 打开去重后的高相似图片页。审阅期间保留命令窗口，按 `Ctrl+C` 即停止服务。该入口不复制或修改图片、标注及去重结果。
