...
for config in "${CONFIGS[@]}"; do
  run_python -c \
    'import sys; from yolo_retraining.config import load_config; load_config(sys.argv[1])' \
    "$config"
done
...
if [[ "$START_TRAINING" -eq 0 ]]; then
  ...
  echo "[Study] rerun with --start-training after reviewing the layouts and configs."
  exit 0
fi
...
set +e
run_curve 2>&1 | tee "$LOG_ROOT/generate_learning_curve.log"
status="${PIPESTATUS[0]}"
set -e
if [[ "$status" -ne 0 ]]; then
