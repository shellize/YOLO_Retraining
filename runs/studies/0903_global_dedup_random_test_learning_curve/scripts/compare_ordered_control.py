...
from compare_randomized_learning_curves import (
    CURVE_CANDIDATES,
    collect_curve,
    resolve_curve_names,
    validate_protocol,
...
ORDERED_SEQUENCE = "GlobalDedupTau099_Ordered__seq-full-cold__stage0-stage7__yolov5s__s42"
...
def validate_ordered(random_rows: list[dict[str, Any]], ordered_rows: list[dict[str, Any]]) -> dict[str, Any]:
    random_protocol = validate_protocol(random_rows)
    ...
    if any(row["test_images"] != 1000 for row in ordered_rows):
        raise ValueError("the ordered control requires 1,000 test images")
...
def plot(random_rows: list[dict[str, Any]], ordered_rows: list[dict[str, Any]]) -> Path:
    ...
    labels = {42: "random split seed 42", 41: "random split seed 41", 43: "random split seed 43"}
    ...
    fig.suptitle("Global Dedup τ=0.99 — Ordered Split Control", fontsize=16, fontweight="bold", y=0.98)
    fig.text(
        0.5,
        0.015,
        "Full Cold | random curves use split seeds 41/42/43 | ordered curve preserves the deduplication input order",
        ha="center",
        color="#52616B",
        fontsize=9.5,
    )
